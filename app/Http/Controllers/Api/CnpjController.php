    ...
    public function show(string $cnpj) {
        ...
        if(!cache()->has('api.cnpj.'.$cnpj)) {
            ...
            $retorno = cache()->remember('api.cnpj.'.$cnpj, 60 * 5, function () use ($retorno) {
                $retorno->put('estado_id', null);
                $retorno->put('cidade_id', null);
                $retorno->put('idpais', null);

                if($retorno->get('uf')) {
                    $estado = Estados::whereUf($retorno->get('uf'))->first();
                    if($estado) {
                        $retorno->put('estado_id', $estado->id);
                ...
                if($retorno->get('municipio')) {
                    $cidade = Cidades::whereNome($retorno->get('municipio'))->first();
                    if($cidade) {
                        $retorno->put('cidade_id', $cidade->id);
                        unset($cidade);
                    }
                }
                ...
                return json_encode($retorno->only('nome_fantasia', 'razao_social', 'cnpj', 'cep', 'uf', 'logradouro', 'bairro', 'numero', 'localidade', 'complemento', 'estado_id', 'cidade_id', 'idpais', 'status'));
            });
        }else{
