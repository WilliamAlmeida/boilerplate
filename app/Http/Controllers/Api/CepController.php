        ...
        if(!cache()->has('api.cep.'.$cep)) {
            ...
            $retorno = cache()->remember('api.cep.'.$cep, 60 * 5, function () use ($retorno) {
                ...
                if($retorno->get('localidade')) {
                    $cidade = Cidades::whereNome($retorno->get('localidade'))->first();
                    if($cidade) {
                ...
                return json_encode($retorno->only('cep', 'uf', 'logradouro', 'bairro', 'numero', 'localidade', 'complemento', 'estado_id', 'cidade_id', 'idpais', 'status'));
            });
        }else{
