<div class="flex justify-between mb-2">
    <div class="flex gap-x-5">
        <x-button label="Hoje" wire:click="goToCurrentMonth" class="btn-outline btn-sm rounded-2xl" />
        <div class="flex items-center gap-x-1">
            <x-button icon="o-chevron-left" wire:click="goToPreviousMonth" class="btn-ghost btn-sm btn-circle" />
            <x-button icon="o-chevron-right" wire:click="goToNextMonth" class="btn-ghost btn-sm btn-circle" />
        </div>
        <h2 class="text-2xl">{{ __($this->startsAt->format('M')).' '.$this->startsAt->format('Y') }}</h2>
    </div>
